...
public:
    ...
    virtual bool hit(const Ray& ray, value_type minimum, value_type maximum, hit_record& record) const;
private:
    Hittable **list_;
    const int list_size_;
};

bool HittableList::hit(const Ray &ray, value_type minimum, value_type maximum, hit_record &record) const {
    hit_record temp_record{.point_at_parameter=Vec3(0.0,0.0,0.0), .normal=Vec3(0.0,0.0,0.0)};
